...
class FileScanner:
    ...
    def _ensure_processors_available(self):
        ...
        # 确保Office处理器可用
        if self.enable_office_ocr and self.office_processor is None:
            try:
                self.office_processor = OfficeProcessor(use_gpu=False)
    ...
    def set_parse_mode(self, mode: str):
        if mode in ("快速", "精细"):
            ...
            try:
                ...
                pass
            try:
                if hasattr(self, 'office_processor') and hasattr(self.office_processor, 'set_mode'):
                    self.office_processor.set_mode(self.parse_mode)
            except Exception:
                pass
    ...
    def scan_files(self, path=None, process_documents=None):
        ...
        try:
                    ...
                    if self._is_supported_file(file):
                        ...
                        try:
                            ...
                            if self._is_current_year_file(file_info):
                                ...
                                # 只有在启用自动OCR时才处理Office文档
                                elif (process_documents and self.enable_office_ocr and 
                                      file_info['extension'] in ['.docx', '.doc', '.xlsx', '.xls']):
                                    office_count += 1
                                    print(f"发现Office文件: {file}")
                                    file_info = self._process_office_file(file_info)
            ...
            if process_documents and self.enable_office_ocr and office_count > 0:
                print(f"其中包含 {office_count} 个Office文件已进行文本提取和向量化")
    ...
    def submit_ocr_tasks(self, file_paths: List[str], progress_callback=None) -> List[str]:
        """提交OCR任务到任务管理器"""
        task_ids = []
        
        for file_path in file_paths:
            if self.task_manager.is_file_locked(file_path):
                print(f"⚠ 文件 {file_path} 正在处理中，跳过")
                continue
            
            file_ext = Path(file_path).suffix.lower()
            
            try:
                if file_ext == '.pdf' and self.enable_pdf_ocr:
                    task_id = self.task_manager.submit_task(
                        file_path, 'pdf', self._process_pdf_file, progress_callback
                    )
                    task_ids.append(task_id)
                elif file_ext in ['.ppt', '.pptx'] and self.enable_ppt_ocr:
                    task_id = self.task_manager.submit_task(
                        file_path, 'ppt', self._process_ppt_file, progress_callback
                    )
                    task_ids.append(task_id)
                elif file_ext in ['.doc', '.docx', '.xls', '.xlsx'] and self.enable_office_ocr:
                    task_id = self.task_manager.submit_task(
                        file_path, 'office', self._process_office_file, progress_callback
                    )
                    task_ids.append(task_id)
                else:
    ...
    def cancel_ocr_task(self, task_id: str) -> bool:
        """取消OCR任务"""
    ...
    def parse_selected_files(self, file_indices):
        ...
        Returns:
            解析结果字典
        """
        if not self.enable_pdf_ocr and not self.enable_ppt_ocr and not self.enable_office_ocr:
        ...
        for idx in file_indices:
            ...
            try:
                ...
                elif file_info['extension'] in ['.docx', '.doc', '.xlsx', '.xls']:
                    print(f"  调用Office处理器...")
                    updated_info = self._process_office_file(file_info)
                    print(f"  Office处理器返回: {updated_info.get('processing_status', '未知状态')}")
                else:
    ...
    def _process_office_file(self, file_info):
        """
        处理Office文档文件：文本提取和向量化存储
        ...
        try:
            # 检查是否启用Office OCR
            if not self.enable_office_ocr:
                print(f"Office OCR已禁用，跳过处理: {file_info['file_name']}")
                file_info['processing_status'] = 'skipped'
                file_info['error_message'] = 'Office OCR已禁用'
                return file_info
            
            office_path = file_info['file_path']
            file_name = file_info['file_name']
            
            print(f"开始处理Office文档: {file_name}")
            print(f"Office文件路径: {office_path}")
            print(f"Office文件大小: {file_info.get('file_size_mb', '未知')} MB")
            
            # 检查文件是否存在
            if not os.path.exists(office_path):
                print(f"错误: Office文件不存在: {office_path}")
                file_info['processing_status'] = 'failed'
                file_info['error_message'] = f'文件不存在: {office_path}'
                return file_info
            
            # 使用Office处理器进行文本提取
            print(f"调用Office处理器...")
            result = self.office_processor.process_office_document(office_path, file_name)
            
            print(f"Office处理器返回结果类型: {type(result)}")
            print(f"Office处理器返回结果: {result}")
            print(f"结果状态: {result.get('status', '未知')}")
            
            # 检查结果结构
            if not isinstance(result, dict):
                print(f"错误: Office处理器返回结果不是字典类型: {type(result)}")
                file_info['processing_status'] = 'failed'
                file_info['error_message'] = f'Office处理器返回结果类型错误: {type(result)}'
                return file_info
            ...
            if result.get('status') == 'success':
                ...
                if not full_text.strip():
                    file_info['processing_status'] = 'no_text'
                    file_info['error_message'] = '无文本内容'
                    print(f"✗ Office文件无文本内容: {file_name}")
                else:
                    # 更新文件信息
                    file_info['processing_status'] = 'success'
                    file_info['text_content'] = full_text
                    file_info['file_type'] = result.get('file_type', 'unknown')
                    file_info['summary'] = result.get('summary', '')
                    file_info['keywords'] = result.get('keywords', [])
                    file_info['hybrid_summary'] = result.get('hybrid_summary', '')
                    file_info['markdown_content'] = result.get('markdown_content', '')
                    ...
                    if result.get('file_type') in ['docx', 'doc']:
                        file_info['total_paragraphs'] = result.get('total_paragraphs', 0)
                        file_info['total_tables'] = result.get('total_tables', 0)
                    elif result.get('file_type') in ['xlsx', 'xls']:
                        file_info['total_sheets'] = result.get('total_sheets', 0)
                        file_info['total_cells'] = result.get('total_cells', 0)
                    
                    print(f"✓ Office文件处理成功: {file_name}")
            else:
                file_info['processing_status'] = 'failed'
                file_info['error_message'] = result.get('message', '未知错误')
                print(f"✗ Office文件处理失败: {file_name}, 错误: {result.get('message', '未知错误')}")
                return file_info
                
        except Exception as e:
            print(f"✗ Office文件处理出错: {file_info.get('file_name', 'unknown')}, 错误: {e}")
            print(f"错误类型: {type(e)}")
            import traceback
            traceback.print_exc()
            file_info['processing_status'] = 'failed'
        ...
        # 将OCR结果保存为独立JSON，便于文件详情查看（Office）
        try:
            from pathlib import Path as _Path
            import json as _json
            ocr_dir = _Path('data') / 'ocr'
            ...
            ocr_payload = {
                ...
                'keywords': file_info.get('keywords', []),
                'text_preview': preview,
                'total_paragraphs': file_info.get('total_paragraphs', 0),
                'total_tables': file_info.get('total_tables', 0),
                'total_sheets': file_info.get('total_sheets', 0),
                'total_cells': file_info.get('total_cells', 0)
            }
            with open(ocr_path, 'w', encoding='utf-8') as f:
                _json.dump(ocr_payload, f, ensure_ascii=False, indent=2)
            file_info['ocr_data_path'] = str(ocr_path)
        except Exception as _e:
            print(f"写入Office OCR详情JSON失败: {file_name}, 错误: {_e}")
    ...
    def get_statistics(self):
        ...
        # PDF、PPT和Office处理统计
        document_stats = {}
        if self.enable_pdf_ocr or self.enable_ppt_ocr or self.enable_office_ocr:
            ...
            office_files = [f for f in self.scanned_files if f['extension'] in ['.docx', '.doc', '.xlsx', '.xls']]
            
            document_stats = {
                'total_pdfs': len(pdf_files),
                'total_ppts': len(ppt_files),
                'total_office': len(office_files),
                ...
                'office_processed': len([f for f in office_files if f.get('processing_status') == 'success']),
                ...
                'office_failed': len([f for f in office_files if f.get('processing_status') == 'failed']),
    ...
    def search_document_content(self, query, top_k=5):
        ...
        try:
            ...
            document_files = [f for f in self.scanned_files 
                            if f['extension'] in ['.pdf', '.ppt', '.pptx', '.docx', '.doc', '.xlsx', '.xls'] 
...
if __name__ == "__main__":
    ...
    # 初始化扫描器，启用PDF、PPT和Office OCR功能
    scanner = FileScanner(enable_pdf_ocr=True, enable_ppt_ocr=True, enable_office_ocr=True, use_gpu=False, use_milvus=False)
    ...
    if files:
        ...
        if 'document_processing_stats' in stats:
            ...
            print(f"  Office文件: {doc_stats['total_office']} 个")
            print(f"  PDF处理成功: {doc_stats['pdf_processed']} 个")
            print(f"  PPT处理成功: {doc_stats['ppt_processed']} 个")
            print(f"  Office处理成功: {doc_stats['office_processed']} 个")
